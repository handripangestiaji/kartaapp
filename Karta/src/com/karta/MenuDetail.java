...
public class MenuDetail extends Activity implements ISideNavigationCallback {
	...
	private ArrayList<CommentModel> arraylist = new ArrayList<CommentModel>();
	private MenuModel menuDetail = new MenuModel();
            ...
            public void onSuccess(String response) {
                try {                		
        	        ...
        	        mIndicator.setViewPager(pager);
           	    	...
           	    	String[] image_user={
           		    	 "http://karta.dreamcube.co.id/user/p1.png",
           		    	 "http://karta.dreamcube.co.id/user/p2.png",
           		    	 "http://karta.dreamcube.co.id/user/p3.png",
           		    	 "http://karta.dreamcube.co.id/user/p4.png"
           	    	};
           	    	    	
           	    	
                  	for (int i = 0; i < image_user.length; i++) 
            		{                  		
                  		UserModel user = new UserModel("A", image_user[i]);
                  		
               			CommentModel tempComment = new CommentModel();
                  		tempComment.setComment(comment[i]);                  		
                  		tempComment.setRating(rating[i]);
                  		tempComment.setUser(user);
               			
               			arraylist.add(tempComment);                   			
            		}
                  	
                	adapter=new ListCommentAdapter(mContext, arraylist);
		...
		public void onClick(View v) {
                switch(v.getId()){
                    ...
                    case R.id.btn_view_business:{                            
                        Intent intent = new Intent(MenuDetail.this, BusinessDetail.class);
    					intent.putExtra("IdRestaurant", menuDetail.getRestaurant().getId());
    					intent.putExtra("RestaurantName", menuDetail.getRestaurant().getName());
    					intent.putExtra("RestaurantAddress", menuDetail.getRestaurant().getAddress());
                    ...
                    case R.id.btn_leave_review:{                            
    					...
    					intent.putExtra("IdMenu", menuDetail.getId());
    					intent.putExtra("MenuName", menuDetail.getName());
    					intent.putExtra("RestaurantName", menuDetail.getRestaurant().getName());
    					intent.putExtra("RestaurantAddress", menuDetail.getRestaurant().getAddress());
                        startActivity(intent);          
